		...
		m.Group("/repos", func() {
			...
			m.Group("/{username}/{reponame}", func() {
				...
				m.Group("/actions", func() {
					...
					m.Post("/jobs/{job_id}/rerun", reqToken(), reqRepoWriter(unit.TypeActions), repo.RerunActionJob)
					m.Group("/runs", func() {
							...
							m.Group("/{id}", func() {
								...
								m.Group("/comments", func() {
									...
									m.Group("/{comment}", func() {
										...
										m.Combo("/resolution").
											Post(reqToken(), mustNotBeArchived(), repo.ResolvePullReviewComment).
											Delete(reqToken(), mustNotBeArchived(), repo.UnresolvePullReviewComment)
